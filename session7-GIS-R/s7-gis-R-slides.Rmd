...
extract(environment, abra[,c('decimalLongitude', 'decimalLatitude')])
```
...
meanss <- layers %>% filter(is_surface & grepl('mean[_]', layer_code)) %>% select(layer_code)
corr <- layers_correlation(meanss)
...
## Overlap with polygons e.g. EEZ/MPA using sf and mregions
    
```{r mregion_advanced, eval=FALSE}
get_features <- function(mrid) {
  ...
  features <- list()
  for(wms in wmsinfo) {
    if(!grepl("wms[?]$", wms$url) && !grepl("/arcgis/services/", wms$url)) {
      ...
      } else if(grepl("geoportal[.]gc[.]ca", wms$url)) {
        wfs_url <- sub("/arcgis/services/", "/arcgis/rest/services/", wms$url)
        wfs_url <- sub("/MapServer/WMSServer[?]$", "/MapServer/0/query?f=geojson&where=", wfs_url)
        wfs_url <- paste0(wfs_url, wms$featureName," = '", wms$value, "'")
      } else {
      ...
      tryCatch({
          ...
          } else if(sf::st_bbox(ft)[1] < -180 || sf::st_bbox(ft)[2] < -90 ||
                    sf::st_bbox(ft)[3] > 180 || sf::st_bbox(ft)[4] > 90) {
...
simplify_canadian_features <- function(ft, dTolerance = 50) {
  ft <- sf::st_transform(ft, 3347) # http://spatialreference.org/ref/epsg/nad83-statistics-canada-lambert/
  ft <- sf::st_simplify(ft, dTolerance)
  return(sf::st_transform(ft, 4326))
}

# Strait of Georgia bioregion
strait_of_georgia_geom <- get_features(mrid = 50171) # http://marineregions.org/gazetteer.php?p=details&id=50171
simple <- simplify_canadian_features(strait_of_georgia_geom, 50) # simplify with a tolerance of 50 meters
# All OBIS species records in the Strait of Georgia since 2010
strait_of_georgia_records <- robis::occurrence(geometry = sf::st_as_text(sf::st_geometry(simple)), startdate = '2010-01-01')
strait_of_georgia_records <- strait_of_georgia_records[!is.na(strait_of_georgia_records$species),]

library(leaflet)

leaflet(strait_of_georgia_geom) %>%
  addProviderTiles("OpenStreetMap.BlackAndWhite") %>% 
  addPolygons(color = "#4488BB", weight = 1, smoothFactor = 0.5, opacity = 1.0, fillOpacity = 0.5)  %>%
  addCircleMarkers(~decimalLongitude, ~decimalLatitude, popup = ~scientificName, data = strait_of_georgia_records,
                   radius = 3, weight = 1, fillColor = "red", color = "red", opacity = 1, fillOpacity = 0.1)
